...
async def async_setup_platform(
    ...
    """Set up Plate Recognizer via platform discovery (legacy image_processing style)."""
...
class PlateRecognizerEntity(ImageProcessingEntity):
    ...
    def __init__(
        ...
        # Build stable name and unique_id from camera slug
    ...
    def _handle_response(self, payload: dict[str, Any], image: bytes) -> None:
        ...
        for vehicle in results:
            ...
            plate_info: dict[str, Any] = {
                ...
                ATTR_REGION: vehicle.get("region", {}).get("code", "unknown"),
                ATTR_VEHICLE_TYPE: vehicle.get("vehicle", {}).get("type", "unknown"),
            }

            if self._mmc:
                vdata = vehicle.get("vehicle", {})
                try:
                    plate_info[ATTR_ORIENTATION] = vdata.get("orientation", [{}])[0].get("orientation", "unknown")
                    plate_info[ATTR_MMC] = {
    ...
    async def _async_save_image(self, image: bytes, had_detections: bool) -> None:
        ...
        if self._always_save_latest:
            await self.hass.async_add_executor_job(_write, folder / f"{camera_slug}_latest.jpg", image)
        if self._save_timestamped and had_detections:
            await self.hass.async_add_executor_job(_write, folder / f"{camera_slug}_{timestamp}.jpg", image)
