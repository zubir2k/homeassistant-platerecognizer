...
class PlateRecognizerEntity(ImageProcessingEntity):
    ...
    def __init__(
        ...
        self._camera_entity: str = config[CONF_CAMERA_ENTITY]
    ...
    @property
    def name(self) -> str:
        camera_name = self._camera_entity.replace("camera.", "").replace("_", " ").title()
        return f"Plate Recognizer {camera_name}"
    ...
    @property
    def state(self) -> int:
        """Number of plates detected in the last scan."""
        return self._state
    ...
    async def async_process_image(self, image: bytes) -> None:
        """Process an image via the Plate Recognizer API."""
        session = async_get_clientsession(self.hass)

        data = aiohttp.FormData()
        data.add_field("upload", io.BytesIO(image), filename="image.jpg", content_type="image/jpeg")
        ...
        self._process_result(payload, image)

    # ------------------------------------------------------------------ #
    # Result processing                                                     #
    # ------------------------------------------------------------------ #

    def _process_result(self, payload: dict[str, Any], image: bytes) -> None:
        """Parse API payload and update entity state."""
        ...
        for vehicle in results:
            ...
            if candidates:
                ...
                plate_str = best.get("plate", plate_str)
                confidence = best.get("score", confidence)

            plate_info[ATTR_PLATE] = plate_str.upper()
            plate_info[ATTR_CONFIDENCE] = round(confidence, 3)

            region_data = vehicle.get("region", {})
            plate_info[ATTR_REGION] = region_data.get("code", "unknown")

            vehicle_data = vehicle.get("vehicle", {})
            plate_info[ATTR_VEHICLE_TYPE] = vehicle_data.get("type", "unknown")

            if self._mmc:
                try:
                    plate_info[ATTR_ORIENTATION] = vehicle_data.get("orientation", [{}])[0].get("orientation", "unknown")
                    plate_info[ATTR_MMC] = {
                        "make": vehicle_data.get("make", [{}])[0].get("name", "unknown"),
                        "model": vehicle_data.get("model", [{}])[0].get("name", "unknown"),
                        "color": vehicle_data.get("color", [{}])[0].get("name", "unknown"),
                    }
                except (IndexError, KeyError, TypeError):
                    pass

            self._plates.append(plate_info)
            detected_plates.append(plate_str.upper())

            # Fire per-vehicle event
            self.hass.bus.fire(
                EVENT_VEHICLE_DETECTED,
                {
        ...
        # Update watched plate fuzzy matching
        if self._watched_plates:
        ...
        if self._save_folder and (results or self._always_save_latest):
            self.hass.async_create_task(
                self._async_save_image(image, bool(results))
            )
    ...
    def _fuzzy_match(watched: str, detected: str, tolerance: int = 2) -> bool:
        """Return True if Hamming-like distance is within tolerance (same length preferred)."""
    ...
    async def _async_save_image(self, image: bytes, had_detections: bool) -> None:
        ...
        camera_slug = self._camera_entity.replace("camera.", "").replace(".", "_")

        if self._always_save_latest:
            latest_path = folder / f"{camera_slug}_latest.jpg"
            async with aiofiles.open(latest_path, "wb") as f:
                await f.write(image)

        if self._save_timestamped and had_detections:
            ts_path = folder / f"{camera_slug}_{timestamp}.jpg"
